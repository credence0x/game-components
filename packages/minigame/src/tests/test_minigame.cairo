...
use snforge_std::{
    ...
    spy_events, EventSpyAssertionsTrait, EventSpyTrait, Event, EventsFilterTrait,
};
...
use crate::tests::mocks::minigame_mock::{
    IMinigameMock, IMinigameMockDispatcher, IMinigameMockDispatcherTrait, IMinigameMockInit,
...
fn deploy_minigame_mock(supports_settings: bool, supports_objectives: bool) -> ContractAddress {
...
fn test_basic_minigame_functionality() {
    let contract_address = deploy_minigame_mock(true, true);
...
#[test]
fn test_mint_basic() {
    let contract_address = deploy_minigame_mock(false, false);
    let minigame = IMinigameDispatcher { contract_address };
...
fn test_mint_with_settings_supported() {
    let contract_address = deploy_minigame_mock(true, false);
    let minigame = IMinigameDispatcher { contract_address };
    let mock = IMinigameMockDispatcher { contract_address };
    let player_addr = contract_address_const::<PLAYER_ADDRESS>();
...
fn test_mint_with_objectives_supported() {
    let contract_address = deploy_minigame_mock(false, true);
    let minigame = IMinigameDispatcher { contract_address };
    let mock = IMinigameMockDispatcher { contract_address };
    let player_addr = contract_address_const::<PLAYER_ADDRESS>();
...
#[test]
fn test_src5_interface_registration_selective() {
    ...
    assert!(
        !src5.supports_interface(IMINIGAME_OBJECTIVES_ID),
        "Should not support IMinigameObjectives interface",
    );
}
...
fn test_objectives_functionality() {
    ...
    // Test getting objectives for token
    let token_objectives = objectives.objectives(token_id);
    assert_eq!(token_objectives.len(), 1);

    let objective = *token_objectives.at(0);
    assert_eq!(objective.name, "Score Target");
    assert_eq!(objective.value, "Score Above 200");
...
#[test]
fn test_game_details_functionality() {
    let contract_address = deploy_minigame_mock(false, false);
    let details = IMinigameDetailsDispatcher { contract_address };
    let minigame = IMinigameDispatcher { contract_address };
    let player_addr = contract_address_const::<PLAYER_ADDRESS>();
    ...
    let description = details.token_description(token_id);
    assert!(description.len() > 0, "Token description should not be empty");

    // Test game details
    let game_details = details.game_details(token_id);
    assert_eq!(game_details.len(), 1, "Should have one game detail");

    let detail = *game_details.at(0);
    assert_eq!(detail.name, "Test Game Detail");
}
...
fn test_multiple_objectives() {
    ...
    mock.create_objective_score(100); // objective 1
    mock.create_objective_score(200); // objective 2
    mock.create_objective_score(300); // objective 3

    // Mint token with multiple objectives
    let token_id = minigame
        .mint(
            Option::Some(PLAYER_NAME),
            Option::None,
    ...
    // Verify all objectives are returned
    let token_objectives = objectives.objectives(token_id);
    assert_eq!(token_objectives.len(), 3, "Should have 3 objectives");

    // Play game and achieve medium score
    mock.start_game(token_id);
    mock.end_game(token_id, 250);

    // Check objective completion
    assert!(objectives.completed_objective(token_id, 1), "First objective should be completed");
    assert!(objectives.completed_objective(token_id, 2), "Second objective should be completed");
    assert!(
        !objectives.completed_objective(token_id, 3), "Third objective should not be completed",
    );
}
...
fn test_soulbound_token() {
    ...
    assert!(token_id > 0, "Soulbound token should be minted successfully");
}
