...
pub mod TokenComponent {
    ...
    use game_components_minigame::interface::{IMINIGAME_ID, IMinigameDispatcher, IMinigameDispatcherTrait, IMinigameTokenDataDispatcher, IMinigameTokenDataDispatcherTrait};
    use game_components_minigame::extensions::objectives::interface::{IMINIGAME_OBJECTIVES_ID, IMinigameObjectivesDispatcher, IMinigameObjectivesDispatcherTrait};
    ...
    > of IMinigameToken<ComponentState<TContractState>> {
        ...
        fn mint(
            ...
            let (game_id, settings_id, objectives_count) = match game_address {
                Option::Some(game_address) => {
                    ...
                    assert!(game_src5_dispatcher.supports_interface(IMINIGAME_ID), "MinigameToken: Game does not support IMinigame");

                    // Check if the token contract supports multi-game
                    let mut game_id = 0;
                    let supports_multi_game_token = src5_component.supports_interface(IMINIGAME_TOKEN_MULTIGAME_ID);
                    if supports_multi_game_token {
                        ...
                        let game_metadata: GameMetadata = multi_game_component.get_game_metadata(game_id);
                        ...
                        // If not a multi-game token, then we need to check game address is same as initialized game address
                        assert!(game_address == self.game_address.read(), "MinigameToken: Selected game address does not match initialized game address");
                    }
                    let settings_id = match settings_id {
                        Option::Some(settings_id) => {       
                            ...
                            assert!(settings_src5_dispatcher.supports_interface(IMINIGAME_SETTINGS_ID), "MinigameToken: Settings contract does not support IMinigameSettings");
                    ...
                    let objectives_count = match objective_ids {
                        Option::Some(objective_ids) => {
                            ...
                            let supports_objectives = src5_component.supports_interface(IMINIGAME_TOKEN_OBJECTIVES_ID);
                            assert!(supports_objectives, "MinigameToken: Contract does not objectives");
                            ...
                            assert!(objectives_src5_dispatcher.supports_interface(IMINIGAME_OBJECTIVES_ID), "MinigameToken: Objectives contract does not support IMinigameObjectives");
                ...
                Option::None => { 
                    ...
                    assert!(src5_component.supports_interface(IMINIGAME_TOKEN_SETTINGS_ID), "MinigameToken: Does not support IMinigameTokenBlank");
                    (0, 0, 0) 
                },
            };
        ...
        fn update_game(ref self: ComponentState<TContractState>, token_id: u64) {
            ...
            let mut erc721_component = get_dep_component_mut!(ref self, ERC721);
            assert!(
                erc721_component.exists(token_id.into()), "MinigameToken: Token id {} not minted", token_id,
            );
            let token_metadata: TokenMetadata = self.token_metadata.entry(token_id).read();
            // If the token is multigame then we need to do the game registry lookup, otherwise we just take the game
            // address itself
            ...
            if supports_objectives {
                let supports_multi_game_token = src5_component.supports_interface(IMINIGAME_TOKEN_MULTIGAME_ID);
                if supports_multi_game_token {
                    game_address = multi_game_component.get_game_address_from_id(token_metadata.game_id);
                }
                ...
                if token_metadata.objectives_count > 0 {
                    ...
                    let minigame_dispatcher = if supports_multi_game_token {
                        ...
                        let game_metadata: GameMetadata = multi_game_component.get_game_metadata(token_metadata.game_id);
                        IMinigameDispatcher {
                            contract_address: game_metadata.contract_address
                        }
                    } else {
                    ...
                    loop {
                        ...
                        let mut objectives_token_component = get_dep_component_mut!(ref self, TokenObjectives);
                        let token_objective: TokenObjective = objectives_token_component.get_objective(token_id, objective_index);
                        ...
                        if is_objective_completed {
                            ...
                            objectives_token_component.set_objective(token_id, objective_index, token_objective);
            ...
            if completed_all_objectives || game_over {
                self.token_metadata.entry(token_id).write(TokenMetadata {
    ...
    > of InternalTrait<TContractState> {
        ...
        fn assert_playable(self: @ComponentState<TContractState>, token_id: u64) {
            ...
            assert!(active && !metadata.completed_all_objectives && !metadata.game_over, "MinigameToken: Token {} is not playable", token_id)
        }

        fn emit_score_update(ref self: ComponentState<TContractState>, token_id: u64, score: u64) {
            self.emit(ScoreUpdate {
                token_id,
                score,
            });
        }
