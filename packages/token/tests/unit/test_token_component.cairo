...
mod MockTokenContract {
    ...
    #[abi(embed_v0)]
    impl MultiGameImpl =
        MultiGameComponent::MinigameTokenMultiGameImpl<ContractState>;
    ...
    #[abi(embed_v0)]
    impl ObjectivesImpl =
        TokenObjectivesComponent::MinigameTokenObjectivesImpl<ContractState>;
    ...
    impl ERC721HooksImpl of ERC721Component::ERC721HooksTrait<ContractState> {
        fn before_update(
            ...
            let token_metadata = contract_state
                .token
                .token_metadata
            ...
            if token_metadata.soulbound {
                contract_state
                    .soulbound
                    .before_update(auth, Zero::zero(), to, token_id.try_into().unwrap());
            }
        }
...
fn test_mint_basic_token() {
    ...
    let (token_address, _) = token_contract
        .deploy(@array![Option::Some(minigame_address).into()])
        .unwrap();
...
fn test_mint_sequential_tokens() {
    ...
    // Mint third token
    let token_id_3 = token_dispatcher
        .mint(
            Option::Some(minigame_address),
            Option::None,
            Option::None,
...
fn test_is_playable_lifecycle_checks() {
    ...
    assert!(
        !token_dispatcher.is_playable(token_id_3), "Token with future start should not be playable",
    );

    // Test 4: Token with past end time
    let token_id_4 = token_dispatcher
        .mint(
            Option::Some(minigame_address),
            Option::None,
            Option::None,
    ...
    assert!(
        !token_dispatcher.is_playable(token_id_4), "Token with past end should not be playable",
    );
}
